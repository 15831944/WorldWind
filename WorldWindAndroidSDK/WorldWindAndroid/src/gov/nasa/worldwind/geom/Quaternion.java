...
public class Quaternion
{
    ...
    public static Quaternion fromMatrix(Matrix matrix)
    {
        ...
        double t = 1.0 + matrix.m[11] + matrix.m[22] + matrix.m[33];
        double x, y, z, w;
        double s;
        ...
        {
			s = 2.0 * Math.sqrt(t);
			x = (matrix.m[32] - matrix.m[23]) / s;
            y = (matrix.m[13] - matrix.m[31]) / s;
            z = (matrix.m[21] - matrix.m[12]) / s;
            w = s / 4.0;
        }
        else if ((matrix.m[11] > matrix.m[22]) && (matrix.m[11] > matrix.m[33]))
        {
			s = 2.0 * Math.sqrt(1.0 + matrix.m[11] - matrix.m[22] - matrix.m[33]);
			x = s / 4.0;
			y = (matrix.m[21] + matrix.m[12]) / s;
			z = (matrix.m[13] + matrix.m[31]) / s;
			w = (matrix.m[32] - matrix.m[23]) / s;
		}
        else if (matrix.m[22] > matrix.m[33])
        {
			s = 2.0 * Math.sqrt(1.0 + matrix.m[22] - matrix.m[11] - matrix.m[33]);
			x = (matrix.m[21] + matrix.m[12]) / s;
			y = s / 4.0;
			z = (matrix.m[32] + matrix.m[23]) / s;
			w = (matrix.m[13] - matrix.m[31]) / s;
		}
        else
        {
			s = 2.0 * Math.sqrt(1.0 + matrix.m[33] - matrix.m[11] - matrix.m[22]);
			x = (matrix.m[13] + matrix.m[31]) / s;
			y = (matrix.m[32] + matrix.m[23]) / s;
			z = s / 4.0;
			w = (matrix.m[21] - matrix.m[12]) / s;
		}
