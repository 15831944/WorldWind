...
public abstract class AbstractShape extends WWObjectImpl implements OrderedRenderable, Highlightable, Movable
{
     ...
     * AVKey#ABSOLUTE}, the shape's geometry is created relative to the specified terrain rather than the terrain used
    ...
    /**
     * Specifies this shape's altitude mode, one of {@link AVKey#ABSOLUTE}, {@link AVKey#RELATIVE_TO_GROUND}, {@link
     * AVKey#CLAMP_TO_GROUND}, or <code>null</code> to indicate the default altitude mode {@link AVKey#ABSOLUTE} should
     * be used.
     * <p/>
     ...
     *
     * @param altitudeMode the altitude mode. The default value is {@link AVKey#ABSOLUTE}.
     */
    public void setAltitudeMode(String altitudeMode)
    {
        if (this.altitudeMode != null ? this.altitudeMode.equals(altitudeMode) : altitudeMode == null)
            return;
    ...
    protected void determineActiveAttributes()
    {
        if (this.isHighlighted())
        {
            if (this.getHighlightAttributes() != null)
                this.activeAttributes.set(this.getHighlightAttributes());
            else
            {
    ...
    protected void beginDrawing(DrawContext dc)
    {
        ...
        if (program == null)
            return; // Message already logged in getDefaultGpuProgram.
    ...
    protected void endDrawing(DrawContext dc)
    {
        ...
        if (program == null)
            return; // Message already logged in getDefaultGpuProgram via beginDrawing.
    ...
    protected GpuProgram getDefaultGpuProgram(GpuResourceCache cache)
    {
        ...
        if (program == null)
        {
            try
            {
                GpuProgram.GpuProgramSource source = GpuProgram.readProgramSource(DEFAULT_VERTEX_SHADER_PATH,
            ...
            {
                String msg = Logging.getMessage("GL.ExceptionLoadingProgram", DEFAULT_VERTEX_SHADER_PATH,
                    DEFAULT_FRAGMENT_SHADER_PATH);
                Logging.error(msg);
    ...
    {
        if (dc.getCurrentProgram() == null)
            return; // No gpu program to run. Message already logged in getDefaultGpuProgram via beginDrawing.
    ...
    {
        if (activeAttrs == null || !activeAttrs.isEnableInterior())
            return;
        ...
        if (!dc.isPickingMode())
        {
            ...
            if (color == null)
                color = defaultAttrs.getInteriorColor();
    ...
    {
        if (activeAttrs == null || !activeAttrs.isEnableOutline())
            return;
        ...
        if (!dc.isPickingMode())
        {
            ...
            if (color == null)
                color = defaultAttrs.getOutlineColor();
        ...
        if (dc.isPickingMode() && activeAttrs.getOutlineWidth() < this.getOutlinePickWidth())
            GLES20.glLineWidth(this.getOutlinePickWidth());
        else
            GLES20.glLineWidth((float) activeAttrs.getOutlineWidth());
    }
    ...
    protected Vec4 computePoint(Terrain terrain, Position position)
    {
        ...
        if (AVKey.CLAMP_TO_GROUND.equals(altitudeMode))
            return terrain.getSurfacePoint(position.latitude, position.longitude, 0d);
        ...
        Iterable<? extends LatLon> positions)
    {
        ...
        if (!AVKey.CLAMP_TO_GROUND.equals(this.getAltitudeMode()))
        {
            ...
            for (LatLon pos : positions)
            {
                ...
                if (AVKey.RELATIVE_TO_GROUND.equals(this.getAltitudeMode()))
