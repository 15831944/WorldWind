...
public class SkyGradientLayer extends AbstractLayer
{
	...
	// protected float[] horizonColor = new float[] { 0.66f, 0.70f, 0.81f, 1.0f
	// }; // horizon color (same as fog)
	protected float[] horizonColor = new float[] { 0.76f, 0.76f, 0.80f, 1.0f }; // horizon
																				// color
	protected float[] zenithColor = new float[] { 0.26f, 0.47f, 0.83f, 1.0f }; // zenith
																				// color
	...
	protected boolean isValid(DrawContext dc)
	{
		...
		// Note: increasing this threshold may produce artifacts like far
		// clipping at very low altitude
	...
	protected Matrix createModelViewMatrix(DrawContext dc)
	{
		...
		// Place sky - TODO: find another ellipsoid friendlier way (the sky dome
	...
	protected Matrix createProjectionMatrix(DrawContext dc)
	{
		...
		// If either the viewport width or height is zero, then treat the
		// dimension as if it had value 1.
	 ...
	 * @param gradientBias
	 *            determines how fast the sky goes from the horizon color to the
	 *            zenith color. A value of <code>1</code> with produce a
	 *            balanced gradient, a value greater then <code>1</code> will
	 *            have the zenith color dominate and a value less then
	 *            <code>1</code> will have the opposite effect.
	 */
	protected ArrayList<float[]> computeSkyDome(DrawContext dc, float radius, double startLat, double endLat, int slices, int stacks, float zenithOpacity, float gradientBias)
	{
		...
		for (int stack = 1; stack < stacks - 1; stack++)
		{
			...
			colorFactorZ = Math.min(1f, linear * gradientBias); // coef zenith
																// color
			colorFactorH = 1 - colorFactorZ; // coef horizon color
			alphaFactor = 1 - Math.pow(linear, 4) * (1 - zenithOpacity); // coef
																			// alpha
																			// transparency
			// top vertex
			...
			colorFactorZTop = Math.min(1f, linearTop * gradientBias); // coef
																		// zenith
																		// color
			colorFactorHTop = 1 - colorFactorZTop; // coef horizon color
			alphaFactorTop = 1 - Math.pow(linearTop, 4) * (1 - zenithOpacity); // coef
																				// alpha
																				// transparency
			// Draw stack
