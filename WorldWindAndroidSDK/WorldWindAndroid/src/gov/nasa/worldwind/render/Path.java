 ...
 * {@link AVKey#ABSOLUTE}, the altitudes are considered as height above the ellipsoid. If the altitude mode is {@link
 * AVKey#RELATIVE_TO_GROUND}, the altitudes are added to the elevation of the terrain at the position. If the altitude
 * mode is {@link AVKey#CLAMP_TO_GROUND} the altitudes are ignored.
 * <p/>
 ...
 * attributes are used. See {@link #DEFAULT_INTERIOR_COLOR}, {@link #DEFAULT_OUTLINE_COLOR}, and {@link
...
public class Path extends AbstractShape
{
    ...
    protected void determineActiveAttributes()
    {
        ...
        if (this.activeAttributes != null && this.activeAttributes.isEnableInterior() != isDrawInterior)
    ...
    protected void addOrderedRenderable(DrawContext dc)
    {
        ...
        {
            dc.addOrderedRenderableToBack(this); // Specify that this Path should appear behind other ordered objects.
        }
        else
        {
    ...
    protected void computePath(DrawContext dc, List<Position> positions, PathData pathData)
    {
        ...
        else if (AVKey.RELATIVE_TO_GROUND.equals(this.getAltitudeMode()))
        ...
        PathData pathData)
    {
        ...
        if (dc.getVerticalExaggeration() != 1)
        {
            ...
            for (Position pos : positions)
            {
                Vec4 pt = globe.computePointFromPosition(pos.latitude, pos.longitude, ve * pos.elevation);
                path.put((float) (pt.x - referencePoint.x));
                path.put((float) (pt.y - referencePoint.y));
                path.put((float) (pt.z - referencePoint.z));
    ...
    {
        Vec4 referencePoint = pathData.getReferencePoint();
        Vec4 pt = dc.getVisibleTerrain().getSurfacePoint(position.latitude, position.longitude, 0d);
        path.put((float) (pt.x - referencePoint.x));
        path.put((float) (pt.y - referencePoint.y));
        path.put((float) (pt.z - referencePoint.z));
        ...
        for (double s = 0, p = 0; s < 1; )
        {
            ...
            if (s >= 1)
            {
                ...
                if (interpolatedColor != null)
                    interpolatedColor.set(colorB);
            }
            else if (this.pathType == AVKey.RHUMB_LINE || this.pathType == AVKey.LINEAR)
            {
                ...
                if (interpolatedColor != null)
                    Color.interpolate(s, colorA, colorB, interpolatedColor);
            }
            else // GREAT_CIRCLE
            {
                if (segmentAzimuth == null)
            ...
            this.addTessellatedPosition(pos, interpolatedColor, s >= 1 ? ordinalB : null, pathData);
    ...
    protected Extent computeExtent(PathData current)
    {
        ...
        Box box = Box.computeBoundingBox(current.renderedPath, current.vertexStride);
